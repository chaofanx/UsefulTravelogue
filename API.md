...
| remark | string | 否 | 备注（高级会员专属，普通用户携带非空备注返回 403） |
| date | string | 否 | 日期，默认当天（支持补记前一天等任意日期） |
| time | string | 否 | 时间，默认当前时分 |
| icon | string | 否 | 自定义图标 emoji，仅「其他」分类生效（可选列表见系统模块） |
| exclude_from_split | boolean | 否 | 不计入分账（高级会员专属，普通用户携带 true 返回 403） |

**业务逻辑：**
- 后端根据 `category` 自动填充 `icon` 字段；「其他」分类可传自定义 emoji 覆盖
- `exclude_from_split = true` 的账单仍计入总支出/分类统计，但不参与分账与成员收支计算
- 金额保留两位小数
...
> 🟡 调用时机：分账统计页 — 点击「导出为 Excel 表格」（高级会员专属，普通用户返回 403，前端不显示该按钮）
...
```json
{
  "code": 0,
  "data": {
    "about": "好用旅记是一款简单好用的多人旅行记账工具。\n版本：1.0.0",
    "version": "1.0.0"
  }
}
```

内容可在管理后台「系统设置」中编辑（存储于 system_configs 表）。

### 24. 获取「其他」分类可选 emoji

> 🟡 调用时机：记一笔 — 选中「其他」分类时展示可更换的图标（无需登录）
...
| 功能 | 普通用户 | 高级会员 |
|------|----------|----------|
| 创建账本 | 仅 1 个（删除后可重建），超出返回 403 | 无限制 |
| 账单备注 | 禁止（前端不显示输入框，后端拦截） | 可用 |
| 单笔不计入分账 | 禁止（前端不显示开关，后端拦截） | 可用 |
| 导出 Excel | 禁止（前端不显示按钮，后端返回 403） | 可用 |

用户类型由管理后台（`/admin`，HTTP Basic 认证）的「用户管理」页修改。
